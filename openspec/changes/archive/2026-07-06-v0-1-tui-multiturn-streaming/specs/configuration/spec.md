...
### Requirement: Config schema distinguishes both backends
The system MUST validate that the YAML contains configuration blocks for BOTH `anthropic` and `openai` backends, even when only one is active, so the user can switch backends by editing one field.

#### Scenario: Both backend blocks required
- **WHEN** the YAML is loaded
- **THEN** the schema MUST require both `anthropic:` and `openai:` sections, each with `api_key` and `model` fields

#### Scenario: Switching backend is a single-field edit
- **WHEN** the user edits only the `backend:` field in `config.yaml` from `anthropic` to `openai` (or vice versa) and restarts
